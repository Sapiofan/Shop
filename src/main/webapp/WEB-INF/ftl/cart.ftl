...
<aside id="sidebar-cart">
    <main>
        ...
        <h2 id="cart-count-h2">Shopping Bag <span id="cart-count-top" class="count">${cartProducts?size!0}</span></h2>
        <div id="pr">
            <ul id="products" class="products">
                <#if cartProducts?has_content>
                    <#list cartProducts as product>
                        <li class="product">
                        ...
                        <span class="product-image">
                            ...
                            <a href="/product/${product.product.id}">
                                ...
                                <span class="qty-price">
                                    <span class="qty">
                                        <button onclick="subtractTotal(${product.product.price}, 'qty-${product.product.id}')"
                                                class="minus-button" id="minus-button-${product.product.id}">-</button>
                                        <input data-onload="disableInput('qty-${product.product.id}')" type="number"
                                               id="qty-${product.product.id}" class="qty-input" step="1" min="1"
                                               max="10" name="qty-input" value="${product.quantity}" pattern="[0-9]*"
                                               title="Quantity" inputmode="numeric">
                                        <button onclick="addTotal(${product.product.price}, 'qty-${product.product.id}')"
                                                class="plus-button" id="plus-button-${product.product.id}">+</button>
                                    </span>
                                    <span class="price">${product.product.price}$</span>
                                </span>
                            </span>
                            </a>
                            <button type="button" onclick="cartChanges('/deleteCartProduct/${product.product.id}')"
                                    class="remove-button"><span class="remove-icon">X</span></button>
                        </li>
                    </#list>
                </#if>
            </ul>

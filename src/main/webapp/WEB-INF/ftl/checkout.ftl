...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<@spring.url '${css}/checkout.css'/>"/>
...
<main id="main-checkout">
    ...
    <div id="cart-container" class="cart-container">
        <h3>Cart</h3>
        <div class="products">
            <#if cartProducts?has_content && cartProducts?size != 0>
                <#list cartProducts as product>
                    <div class="product">
                        ...
                        <div class="input-group">
                            <div class="input-group-button">
                                <button onclick="checkoutChanges('/subtractAdditionalProduct/${product.product.id}')"
                                        type="button" class="op" data-quantity="minus" data-field="quantity">
                                    <i class="fa fa-minus" aria-hidden="true"></i>
                                </button>
                            </div>
                            <input disabled id="qty-${product.product.id}"
                                   class="input-group-field quantity" min="1" max="1000" type="number" name="quantity"
                                   value="${product.quantity}">
                            <div class="input-group-button">
                                <button type="button"
                                        onclick="checkoutChanges('/addAdditionalProduct/${product.product.id}')"
                                        class="op" data-quantity="plus" data-field="quantity">
                                    <i class="fa fa-plus" aria-hidden="true"></i>
                                </button>
                            </div>
                        </div>
